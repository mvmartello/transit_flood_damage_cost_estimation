...
async function flood_depths_out() {
  ...
  depth_x_processing(GL_B_depth, GL_B_depth_x);
  depth_x_processing(GL_C_depth, GL_C_depth_x);
  depth_x_processing(GL_D_depth, GL_D_depth_x);
  depth_x_processing(GL_E_depth, GL_E_depth_x);
...
async function in_segment_interpolate(md, bd, x_i,key_i) {
  ...
  //If the depth is less than 0.49
  if (depth < 0.49) {
    //set equal to 0
    depth = 0;
  }
  ...
  //Push the result to the correct output variable
    ...
    light_rail_flood_depths.push(depth);
    //Push to depth variable
    GL_B_depth.push(depth);  
  } else if (key_i == 'GL_C') {
    //Green Line (light rail)
    light_rail_flood_depths.push(depth);
    //Push to depth variable
    GL_C_depth.push(depth);  
  } else if (key_i == 'GL_D') {
    //Green Line (light rail)
    light_rail_flood_depths.push(depth);
    //Push to depth variable
    GL_D_depth.push(depth);  
  } else if (key_i == 'GL_E') {
    //Green Line (light rail)
    light_rail_flood_depths.push(depth);
    //Push to depth variable
    GL_E_depth.push(depth);  
...
async function poi_depth_processing(scenario_in = 1) {
    ...
    for (const key of key_list) {
        ...
        for (poi = 0; poi < (sys_poi_out[key].length - 1); poi++) {
          ...
          if (sys_poi_flooded.includes(sys_poi_out[key][poi].POI) == false && sys_poi_out[key][poi].POI.includes("switch") == false && (sys_poi_out[key][poi]["Maximum HGL Feet"] - sys_poi_out[key][poi].invert_elev) > 0.1) {
              ...
              //If the depth is greater than 0.49ft,
              if (depth_temp > 0.49) {
    ...
    //Check to see if any Yards are flooded
    for (var itr = 0; itr < at_grade_flood_elev.length; itr++) {
      //If the at-grade location is a rail yard, check to see if it's flooded
      if(at_grade_flood_elev[itr].Line == "Yard" && at_grade_flood_elev[itr][scenario_in] > 0) {
        //Push to flood variables
        sys_poi_depth.push({POI: at_grade_flood_elev[itr].STA_1, depth: at_grade_flood_elev[itr][scenario_in], x: NaN, Line: at_grade_flood_elev[itr].Line});
        sys_poi_flooded.push(at_grade_flood_elev[itr].STA_1);  
      }
    }
    return sys_poi_depth;
}
